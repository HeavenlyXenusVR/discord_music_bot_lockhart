...
![Lockhart bot icon](assets/bot-icon.png)

Lockhart is one node in the 13-bot Discord music fleet. Named after Tifa Lockhart, Strife's longtime companion node. It owns its queue tables, reports playback state to the shared swarm database, and is controlled directly from Discord, Aria, and SwarmPanel.

## Identity

- Discord display name: **GWS Music Bot (Lockhart)**
- Internal node name: `lockhart`
- Command namespace: `/lockhart_main_*`
...
All commands are namespaced as `/lockhart_main_<command>`.
