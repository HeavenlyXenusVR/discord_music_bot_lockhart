--[[
  LOCKHART -- swarm music node.
  LuaJIT port of lockhart.py, node `lockhart` of the 13-bot swarm.

  lockhart.py is byte-for-byte identical to rhythm.py, alucard.py, maestro.py,
  melodic.py, and tunestream.py aside from the bot-name/table-prefix/env-prefix
  substitution (confirmed by diffing lockhart.py against each of them during
  this port: after normalizing both bot names to a common placeholder, the
  only remaining diff hunks are the BOT_STAGGER_SLOTS dict's per-bot slot
  numbers and a handful of incidental blank-line/whitespace differences --
  zero residual logic diffs) -- the same codebase reskinned under different
  names/branding. Per the orchestrator's guidance, this file was therefore
  built from discord_music_bot_tunestream/tunestream.lua (the most
  recently-built/up-to-date reference at the time of this port, itself built
  from melodic.lua, itself from maestro.lua, itself from alucard.lua -- see
  tunestream.lua's own header for that lineage and its current_position()
  clamping/timescale bugfix vs rhythm.lua) via a mechanical tunestream->lockhart
  rename (case-sensitive sed pass, verified zero residual "tunestream" hits
  afterward), then re-diffed line-by-line against lockhart.py to confirm
  behavior parity.

  UNLIKE every other bot ported so far, discord_music_lockhart started as a
  completely empty Postgres database -- this bot did not exist in the old
  MariaDB deployment, so there was no migrated data and, critically, no
  pre-existing tables for a "tables already exist, just probe them" init_db()
  to lean on. discord_music_bot_strife/strife.lua hit the identical situation
  (also a brand-new database) and already solved it with a SCHEMA_TABLES /
  SCHEMA_INDEXES / bootstrap_schema() block of CREATE TABLE IF NOT EXISTS
  statements authored fresh at the desired Postgres-native end-state; that
  pattern (renamed strife_ -> lockhart_) is reused verbatim below as
  bootstrap_schema(), called from init_db() in place of tunestream.lua's
  no-op probe-insert-then-delete. One extra table not present in strife's
  schema, lockhart_search_cache, was added here since this file (following
  tunestream.lua) keeps the 15-minute DB-backed search cache that strife
  does not.

  Full command-surface parity with lockhart.py: all 59 of its
  `/lockhart_main_*` slash commands are ported (verified name-for-name against
  lockhart.py's @bot.tree.command declarations -- see the diff in the port
  report). lockhart_error_events IS ported here (same as alucard.lua; unlike
  ...
  errors all land in the shared lockhart_error_events table (same table
  Aria/SwarmPanel reads from for the cross-bot error dashboard) and, if
  LOCKHART_ERROR_WEBHOOK_URL is configured, post to a dedicated error webhook,
  matching lockhart.py's _persist_error_event()/send_error_webhook_log() pair.
  LOCKHART_WEBHOOK_URL (operational "node online" style logging) is also
  wired, matching lockhart.py's send_webhook_log().
  ...
  lockhart.py's own source also carries an alucard_status_messages-style
  in-place status-message-edit feature (lockhart_status_messages) that
  ...
  tracking table):
    ...
    - The DAVE-protocol aiohttp monkeypatch (lockhart.py's
    ...
    - Gemini track embeddings/cooccurrence (lockhart_track_embeddings,
      lockhart_track_cooccurrence) -- Auto-DJ below is a simplified
      scoring model (see "Smart Auto-DJ" section) driven by
      lockhart_track_intelligence / lockhart_user_track_affinity counters
      instead of semantic embedding similarity.
    - The Aria/SwarmPanel remote-control bridge (lockhart_swarm_direct_orders,
      lockhart_swarm_overrides, direct_order_listener/aria_command_listener) --
      this bot is controlled directly via its own Discord slash commands only.
    - "Live playlist sync" (lockhart_active_playlist_tracks, the background
      task that re-scans a queued playlist URL for newly-added videos) --
      /lockhart_main_play expands a playlist once, in full, at queue time via
      Lavalink's loadType=playlist response. lockhart_active_playlists (the
      per-guild "what playlist is active" row, not the per-track sync table)
      IS still ported/used the same way rhythm.lua uses it.
    - "Beat-matched mixing" (/lockhart_main_fade mix) falls back to the same
      smooth-fade curve as /lockhart_main_fade fade -- no BPM analysis/matching
      was ported.
    ...
    - The dozen-plus overlapping @tasks.loop coroutines (position updater,
      ...
      persistence, swarm heartbeat + lockhart_metrics, and error-event
...
-- ===========================================================================
-- Env / configuration (mirrors lockhart.py's BOT_ENV_PREFIX = "LOCKHART")
-- ===========================================================================
...
local PREFIX = "LOCKHART"
local TOKEN = env(PREFIX .. "_DISCORD_TOKEN", "")
if TOKEN == "" then
  io.stderr:write("[lockhart] " .. PREFIX .. "_DISCORD_TOKEN is not set; refusing to start.\n")
  os.exit(1)
end
...
local DB_CONFIG = {
  ...
  database = env(PREFIX .. "_DB_NAME", "discord_music_lockhart"),
}
...
if LAVALINK_PASSWORD == "" then
  io.stderr:write("[lockhart] Set " .. PREFIX .. "_LAVALINK_PASSWORD or LAVALINK_PASSWORD before starting lockhart.\n")
  os.exit(1)
end
...
local LOG_FILE = LOG_DIR .. "/lockhart.log"
...
local function logline(level, fmt, ...)
  ...
  local line = string.format("%s %-5s lockhart %s", os.date("%Y-%m-%d %H:%M:%S"), level, msg)
  print(line)
...
local function http_get_json(url)
  ...
  local ok, status = https.request({
    url = url,
    method = "GET",
    headers = { ["User-Agent"] = "swarmlua-lockhart/1.0" },
...
-- default per the README's guidance; lockhart needs neither message content
...
-- ===========================================================================
-- Schema bootstrap. discord_music_lockhart started completely empty (see the
-- header comment) so, unlike every other sibling bot's init_db(), this authors
-- the schema directly at its desired end-state (Postgres-native types) rather
-- than just probing tables a MariaDB migration already created. Pattern lifted
-- from discord_music_bot_strife/strife.lua's SCHEMA_TABLES/SCHEMA_INDEXES/
-- bootstrap_schema(), which hit the same "brand-new database" situation.
-- ===========================================================================

local SCHEMA_TABLES = {
[[CREATE TABLE IF NOT EXISTS lockhart_playback_state (
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
  channel_id BIGINT,
  video_url TEXT,
...
[[CREATE TABLE IF NOT EXISTS lockhart_guild_settings (
  guild_id BIGINT PRIMARY KEY,
  home_vc_id BIGINT,
  volume INTEGER NOT NULL DEFAULT 100,
  loop_mode VARCHAR(10) NOT NULL DEFAULT 'queue',
  filter_mode VARCHAR(20) NOT NULL DEFAULT 'none',
  filter_stack VARCHAR(20),
  dj_role_id BIGINT,
  feedback_channel_id BIGINT,
  transition_mode VARCHAR(10) NOT NULL DEFAULT 'off',
  fade_seconds DOUBLE PRECISION NOT NULL DEFAULT 3.0,
  fade_curve VARCHAR(20) NOT NULL DEFAULT 'smooth',
  custom_speed DOUBLE PRECISION NOT NULL DEFAULT 1.0,
  custom_pitch DOUBLE PRECISION NOT NULL DEFAULT 1.0,
  custom_modifiers_left INTEGER NOT NULL DEFAULT 0,
  dj_only_mode BOOLEAN NOT NULL DEFAULT FALSE,
  stay_in_vc BOOLEAN NOT NULL DEFAULT FALSE
)]],
[[CREATE TABLE IF NOT EXISTS lockhart_queue (
...
[[CREATE TABLE IF NOT EXISTS lockhart_queue_backup (
  id BIGSERIAL PRIMARY KEY,
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
  video_url TEXT,
...
[[CREATE TABLE IF NOT EXISTS lockhart_history (
  id BIGSERIAL PRIMARY KEY,
  guild_id BIGINT NOT NULL,
  video_url TEXT,
  title TEXT,
...
[[CREATE TABLE IF NOT EXISTS lockhart_user_playlists (
  id BIGSERIAL PRIMARY KEY,
...
[[CREATE TABLE IF NOT EXISTS lockhart_track_intelligence (
  guild_id BIGINT NOT NULL,
...
[[CREATE TABLE IF NOT EXISTS lockhart_user_track_affinity (
  guild_id BIGINT NOT NULL,
  user_id BIGINT NOT NULL,
  url_key VARCHAR(64) NOT NULL,
  video_url TEXT,
...
[[CREATE TABLE IF NOT EXISTS lockhart_smart_recommendations (
  id BIGSERIAL PRIMARY KEY,
  guild_id BIGINT NOT NULL,
  requester_id BIGINT,
  seed_title TEXT,
...
[[CREATE TABLE IF NOT EXISTS lockhart_bot_home_channels (
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
  home_vc_id BIGINT,
  PRIMARY KEY (guild_id, bot_name)
)]],
[[CREATE TABLE IF NOT EXISTS lockhart_voice_state (
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
...
[[CREATE TABLE IF NOT EXISTS lockhart_metrics (
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
  voice_connected BOOLEAN NOT NULL DEFAULT FALSE,
  connected_channel_id BIGINT,
  player_connected BOOLEAN NOT NULL DEFAULT FALSE,
  player_playing BOOLEAN NOT NULL DEFAULT FALSE,
  player_paused BOOLEAN NOT NULL DEFAULT FALSE,
  queue_count INTEGER NOT NULL DEFAULT 0,
  backup_queue_count INTEGER NOT NULL DEFAULT 0,
  is_playing_db BOOLEAN NOT NULL DEFAULT FALSE,
  is_paused_db BOOLEAN NOT NULL DEFAULT FALSE,
  position_seconds INTEGER NOT NULL DEFAULT 0,
  duration_seconds INTEGER NOT NULL DEFAULT 0,
  recovery_pending BOOLEAN NOT NULL DEFAULT FALSE,
  heartbeat_age_seconds INTEGER NOT NULL DEFAULT 0,
  lavalink_ready BOOLEAN NOT NULL DEFAULT FALSE,
  last_error TEXT,
  updated_at TIMESTAMPTZ DEFAULT now(),
  PRIMARY KEY (guild_id, bot_name)
)]],
[[CREATE TABLE IF NOT EXISTS lockhart_active_playlists (
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
  playlist_url TEXT,
  known_track_count INTEGER NOT NULL DEFAULT 0,
  requester_id BIGINT,
  channel_id BIGINT,
  PRIMARY KEY (guild_id, bot_name)
)]],
-- Companion to lockhart_active_playlists above -- unlike that table, this one
-- didn't exist at all before playlist_sync_tick() (see its header comment
-- further down) started actually using it.
[[CREATE TABLE IF NOT EXISTS lockhart_active_playlist_tracks (
  guild_id BIGINT NOT NULL,
  bot_name VARCHAR(50) NOT NULL DEFAULT 'lockhart',
  playlist_url TEXT,
...
[[CREATE TABLE IF NOT EXISTS lockhart_swarm_toggles (
  guild_id BIGINT PRIMARY KEY,
  auto_dj BOOLEAN NOT NULL DEFAULT FALSE,
...
[[CREATE TABLE IF NOT EXISTS lockhart_error_events (
  id BIGSERIAL PRIMARY KEY,
...
-- 15-minute DB-backed search cache (search_playables()); not present in
-- strife's schema since strife.lua doesn't cache resolved searches, but
-- tunestream.lua (this file's structural base) does -- see search_cache_get/put.
[[CREATE TABLE IF NOT EXISTS lockhart_search_cache (
...
local SCHEMA_INDEXES = {
  "CREATE INDEX IF NOT EXISTS lockhart_track_intelligence_recent_idx ON lockhart_track_intelligence (guild_id, last_played)",
  "CREATE INDEX IF NOT EXISTS lockhart_track_intelligence_requester_idx ON lockhart_track_intelligence (guild_id, last_requester_id, last_played)",
  "CREATE INDEX IF NOT EXISTS lockhart_user_affinity_recent_idx ON lockhart_user_track_affinity (guild_id, user_id, last_requested)",
  "CREATE INDEX IF NOT EXISTS lockhart_smart_recommendations_recent_idx ON lockhart_smart_recommendations (guild_id, created_at)",
  "CREATE INDEX IF NOT EXISTS lockhart_track_intelligence_global_idx ON lockhart_track_intelligence (url_key, last_played)",
  "CREATE INDEX IF NOT EXISTS lockhart_voice_state_rejoin_idx ON lockhart_voice_state (bot_name, desired_connected, last_seen_at)",
  "CREATE INDEX IF NOT EXISTS lockhart_metrics_status_idx ON lockhart_metrics (bot_name, updated_at)",
  "CREATE INDEX IF NOT EXISTS lockhart_queue_lookup_idx ON lockhart_queue (guild_id, bot_name, id)",
  "CREATE INDEX IF NOT EXISTS lockhart_queue_backup_lookup_idx ON lockhart_queue_backup (guild_id, bot_name, id)",
  "CREATE UNIQUE INDEX IF NOT EXISTS lockhart_queue_track_uid_unique_idx ON lockhart_queue (guild_id, bot_name, track_uid)",
  "CREATE UNIQUE INDEX IF NOT EXISTS lockhart_queue_backup_track_uid_unique_idx ON lockhart_queue_backup (guild_id, bot_name, track_uid)",
  "CREATE INDEX IF NOT EXISTS lockhart_history_guild_played_idx ON lockhart_history (guild_id, played_at)",
  "CREATE INDEX IF NOT EXISTS lockhart_history_requester_idx ON lockhart_history (guild_id, requester_id, played_at)",
  "CREATE INDEX IF NOT EXISTS lockhart_playback_resume_idx ON lockhart_playback_state (bot_name, is_playing, guild_id)",
  "CREATE INDEX IF NOT EXISTS lockhart_playback_track_uid_idx ON lockhart_playback_state (bot_name, track_uid)",
  "CREATE INDEX IF NOT EXISTS lockhart_playlist_bot_idx ON lockhart_active_playlists (bot_name, guild_id)",
  "CREATE INDEX IF NOT EXISTS lockhart_active_playlist_tracks_lookup_idx ON lockhart_active_playlist_tracks (guild_id, bot_name)",
  "CREATE INDEX IF NOT EXISTS lockhart_user_playlist_lookup_idx ON lockhart_user_playlists (user_id, playlist_name)",
  "CREATE INDEX IF NOT EXISTS lockhart_error_events_guild_time_idx ON lockhart_error_events (guild_id, created_at)",
  "CREATE INDEX IF NOT EXISTS lockhart_search_cache_expires_idx ON lockhart_search_cache (expires_at)",
}
...
local function bootstrap_schema()
  for _, sql in ipairs(SCHEMA_TABLES) do
    ...
    if not ok then
      error("lockhart schema bootstrap failed on CREATE TABLE: " .. tostring(err) .. "\nSQL: " .. sql)
    end
  end
  for _, sql in ipairs(SCHEMA_INDEXES) do
    local ok, err = bot.db:query(sql)
    if not ok then
      log_warn("schema index warning (non-fatal): %s -- %s", tostring(err), sql)
    end
  end
  log_info("Postgres schema verified/created (discord_music_lockhart).")
end

local function init_db()
  bootstrap_schema()
  q([[INSERT INTO lockhart_guild_settings (guild_id, volume, loop_mode, filter_mode, transition_mode,
  ...
  q("DELETE FROM lockhart_guild_settings WHERE guild_id = %s", 0)
...
local function get_settings(guild_id)
  ...
  local row = q1("SELECT * FROM lockhart_guild_settings WHERE guild_id = %s", guild_id)
...
local function get_home_channel_id(guild_id)
  local row = q1("SELECT home_vc_id FROM lockhart_bot_home_channels WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
  return row and row.home_vc_id or nil
end

local function get_autodj_enabled(guild_id)
  local row = q1("SELECT auto_dj FROM lockhart_swarm_toggles WHERE guild_id = %s", guild_id)
  if row and row.auto_dj ~= nil then return row.auto_dj end
  return false
end

local function set_autodj_enabled(guild_id, enabled)
  q([[INSERT INTO lockhart_swarm_toggles (guild_id, auto_dj) VALUES (%s, %s)
      ON CONFLICT (guild_id) DO UPDATE SET auto_dj = EXCLUDED.auto_dj]], guild_id, enabled)
end

-- ===========================================================================
-- Discord embed / interaction reply helpers
-- (Bot:reply only does plain content; lockhart's whole personality is embeds,
...
-- ===========================================================================
-- Operational webhook + error events: mirrors lockhart.py's send_webhook_log()/
-- _persist_error_event()/send_error_webhook_log() -- writes into the shared
-- lockhart_error_events table (same table Aria/SwarmPanel reads from for the
...
local function send_webhook_log(title, description, color, username)
  ...
  local ok, body = pcall(function()
    ...
    local payload = cjson.encode({
      username = username or "Node: Lockhart",
...
local function persist_error_event(guild_id, level, error_type, title, description)
  q([[INSERT INTO lockhart_error_events (bot_name, guild_id, error_level, error_type, title, description)
      VALUES ('lockhart', %s, %s, %s, %s, %s)]],
...
local function send_error_webhook(title, description)
  ...
  local ok, body = pcall(function()
    local response_body = {}
    local payload = cjson.encode({
      embeds = { { title = "\xF0\x9F\x94\xB4 Lockhart Error: " .. truncate(title, 200), description = truncate(description, 4000),
                   color = COLOR.red, footer = { text = "Lockhart Music Bot" } } },
    })
    https.request({
...
-- lockhart_error_events / the error webhook, not just process_queue/
...
-- DB-backed search cache (lockhart_search_cache), 15 minute TTL -- mirrors the
-- Python bot's cache table without the extra in-process TTL cache layer.
local function search_cache_get(cache_key)
  local row = q1("SELECT resolved_uri, title FROM lockhart_search_cache WHERE cache_key = %s AND expires_at > NOW()", cache_key)
  if not row then return nil end
  return row.resolved_uri, row.title
end

local function search_cache_put(cache_key, uri, title)
  q([[INSERT INTO lockhart_search_cache (cache_key, resolved_uri, title, cached_at, expires_at)
...
-- ===========================================================================
-- Queue table helpers (lockhart_queue / lockhart_queue_backup)
-- ===========================================================================

local function queue_count(guild_id)
  local row = q1("SELECT COUNT(*)::int AS c FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
  return row and row.c or 0
end

local function snapshot_backup(guild_id)
  q("DELETE FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
  q([[INSERT INTO lockhart_queue_backup (guild_id, bot_name, video_url, title, requester_id, track_uid)
      SELECT guild_id, bot_name, video_url, title, requester_id, track_uid FROM lockhart_queue
      WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC]], guild_id)
end

local function enqueue_track(guild_id, url, title, requester_id, track_uid)
  track_uid = track_uid or new_uid()
  q([[INSERT INTO lockhart_queue (guild_id, bot_name, video_url, title, requester_id, track_uid)
      VALUES (%s, 'lockhart', %s, %s, %s, %s)]], guild_id, url, title, requester_id, track_uid)
  -- track_intelligence: queued_count bump (best-effort learning signal for Auto-DJ)
  local uk = track_key(url, title)
  q([[INSERT INTO lockhart_track_intelligence (guild_id, url_key, video_url, title, queued_count, play_count,
      ...
      ON CONFLICT (guild_id, url_key) DO UPDATE SET
        queued_count = lockhart_track_intelligence.queued_count + 1,
...
local function insert_queue_front(guild_id, url, title, requester_id, track_uid)
  ...
  local rows = q("SELECT id, video_url, title, requester_id, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", guild_id)
  q("DELETE FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
  q("INSERT INTO lockhart_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, 'lockhart', %s, %s, %s, %s)",
...
local function delete_backup_track(guild_id, track_uid, video_url, title)
  if track_uid then
    q("DELETE FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart' AND track_uid = %s", guild_id, track_uid)
  elseif video_url then
    q([[DELETE FROM lockhart_queue_backup WHERE id IN (
          SELECT id FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart' AND video_url = %s LIMIT 1)]],
      guild_id, video_url)
  elseif title then
    q([[DELETE FROM lockhart_queue_backup WHERE id IN (
          SELECT id FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart' AND title = %s LIMIT 1)]],
      guild_id, title)
  end
end
...
local function restore_queue_from_backup(guild_id)
  local rows = q("SELECT video_url, title, requester_id, track_uid FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", guild_id)
  for _, r in ipairs(rows) do
    q("INSERT INTO lockhart_queue (guild_id, bot_name, video_url, title, requester_id, track_uid) VALUES (%s, 'lockhart', %s, %s, %s, %s)",
      guild_id, r.video_url, r.title, r.requester_id, col(r, "track_uid") or new_uid())
  end
...
-- (lockhart_active_playlists existed only as inert per-guild metadata,
...
local function clear_active_playlist(guild_id)
  q("DELETE FROM lockhart_active_playlists WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
  q("DELETE FROM lockhart_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
  playlist_missing_streak[guild_id] = nil
end

-- track_key() itself caps at 64 chars (matches lockhart_track_intelligence's
-- url_key VARCHAR(64)), but lockhart_active_playlist_tracks.track_key is a
...
local function set_active_playlist(guild_id, playlist_url, entries, requester_id, channel_id)
  ...
  q([[INSERT INTO lockhart_active_playlists (guild_id, bot_name, playlist_url, known_track_count, requester_id, channel_id)
  ...
  for idx, t in ipairs(entries) do
    ...
    q([[INSERT INTO lockhart_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, track_uid, video_url, title, requester_id)
        VALUES (%s, 'lockhart', %s, %s, %s, %s, %s, %s, %s)]],
...
-- last-known snapshot (lockhart_active_playlist_tracks), enqueues additions,
...
local function playlist_sync_tick()
  local playlists = q("SELECT guild_id, playlist_url, known_track_count, requester_id, channel_id FROM lockhart_active_playlists WHERE bot_name = 'lockhart'") or {}
  for _, prow in ipairs(playlists) do
    ...
    local ok, err = pcall(function()
      ...
      local previous_rows = q("SELECT track_key, track_uid, video_url, title, requester_id FROM lockhart_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY position_idx ASC", gid) or {}
      ...
      if next(removed_counts) ~= nil then
        ...
        local live_rows = q("SELECT id, video_url, title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", gid) or {}
        for _, lr in ipairs(live_rows) do
          local k = playlist_track_key(lr.video_url, lr.title)
          if budget[k] and budget[k] > 0 then
            budget[k] = budget[k] - 1
            q("DELETE FROM lockhart_queue WHERE id = %s AND guild_id = %s AND bot_name = 'lockhart'", lr.id, gid)
            purged_live = purged_live + 1
          end
        end
        budget = {}
        for k, c in pairs(removed_counts) do budget[k] = c end
        local backup_rows = q("SELECT id, video_url, title FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", gid) or {}
        for _, br in ipairs(backup_rows) do
          local k = playlist_track_key(br.video_url, br.title)
          if budget[k] and budget[k] > 0 then
            budget[k] = budget[k] - 1
            q("DELETE FROM lockhart_queue_backup WHERE id = %s AND guild_id = %s AND bot_name = 'lockhart'", br.id, gid)
      ...
      local all_rows = q("SELECT id, video_url, title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", gid) or {}
      local seen = {}
      for _, qr in ipairs(all_rows) do
        local k = playlist_track_key(qr.video_url, qr.title)
        if current_keys[k] then
          if seen[k] then
            q("DELETE FROM lockhart_queue WHERE id = %s AND guild_id = %s AND bot_name = 'lockhart'", qr.id, gid)
      ...
      local refilled = 0
      local queued_rows = q("SELECT video_url, title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart'", gid) or {}
      local queued_keys = {}
      local queued_n = 0
      ...
      q("DELETE FROM lockhart_active_playlist_tracks WHERE guild_id = %s AND bot_name = 'lockhart'", gid)
      local idx = 0
      for _, r in ipairs(current_rows) do
        q([[INSERT INTO lockhart_active_playlist_tracks (guild_id, bot_name, playlist_url, position_idx, track_key, track_uid, video_url, title, requester_id)
            VALUES (%s, 'lockhart', %s, %s, %s, %s, %s, %s, %s)]],
      ...
      q("UPDATE lockhart_active_playlists SET known_track_count = %s WHERE guild_id = %s AND bot_name = 'lockhart'", idx, gid)
    end)
    if not ok then
...
local function update_stage_topic(guild_id, channel_id, title)
  ...
  if not ok then log_warn("stage/voice topic update failed for %s: %s", tostring(guild_id), tostring(err)) end
end
...
-- Pops the next track from lockhart_queue and starts it. If the queue is empty,
-- tries to restore from lockhart_queue_backup (loop_mode == queue), else runs
...
-- failed resolve just moved to the next row with no requeue), so a short
-- outage could wipe an entire queue in seconds.
...
local function process_queue(guild_id, channel_id)
  ...
  local ok, err = pcall(function()
    local settings = get_settings(guild_id)
    local next_row = q1("SELECT id, video_url, title, requester_id, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT 1", guild_id)
    ...
    if not next_row then
      if settings.loop_mode == "queue" then
        ...
        if restored > 0 then
          ...
          next_row = q1("SELECT id, video_url, title, requester_id, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT 1", guild_id)
        end
      end
    end

    if not next_row then
      q([[UPDATE lockhart_playback_state SET channel_id = NULL, video_url = NULL, title = NULL,
          position_seconds = 0, is_playing = false, is_paused = false, play_session_key = NULL, track_uid = NULL
          WHERE guild_id = %s AND bot_name = 'lockhart']], guild_id)
    ...
    q("DELETE FROM lockhart_queue WHERE id = %s AND guild_id = %s AND bot_name = 'lockhart'", next_row.id, guild_id)
    ...
    if settings.custom_modifiers_left and settings.custom_modifiers_left > 0 then
      ...
      q("UPDATE lockhart_guild_settings SET custom_modifiers_left = custom_modifiers_left - 1 WHERE guild_id = %s", guild_id)
      if settings.custom_modifiers_left - 1 <= 0 then
        q("UPDATE lockhart_guild_settings SET custom_speed = 1.0, custom_pitch = 1.0 WHERE guild_id = %s", guild_id)
      end
    else
    ...
    q([[INSERT INTO lockhart_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title, play_session_key, track_uid)
        VALUES (%s, 'lockhart', %s, %s, 0, true, false, %s, %s, %s)
    ...
    q("INSERT INTO lockhart_history (guild_id, video_url, title, requester_id, played_at) VALUES (%s, %s, %s, %s, NOW())",
      guild_id, track.uri or url, track.title or title, requester_id)
    q([[INSERT INTO lockhart_track_intelligence (guild_id, url_key, video_url, title, queued_count, play_count, finish_count, skip_count, like_count, dislike_count, total_listen_seconds, last_requester_id, first_seen, last_played, updated_at)
        VALUES (%s, %s, %s, %s, 0, 1, 0, 0, 0, 0, 0, %s, NOW(), NOW(), NOW())
        ON CONFLICT (guild_id, url_key) DO UPDATE SET play_count = lockhart_track_intelligence.play_count + 1,
...
local function stop_playback(guild_id)
  local channel_id = (playback[guild_id] and playback[guild_id].channel_id) or get_home_channel_id(guild_id)
  clear_stage_topic(guild_id, channel_id)
...
-- ===========================================================================
-- Smart Auto-DJ (simplified): scores candidates from lockhart_track_intelligence
-- and lockhart_user_track_affinity instead of the Python bot's Gemini-embedding
...
local function record_track_feedback(guild_id, user_id, url, title, liked)
  ...
  q([[INSERT INTO lockhart_track_intelligence (guild_id, url_key, video_url, title, queued_count, play_count, finish_count, skip_count, like_count, dislike_count, total_listen_seconds, first_seen, updated_at)
      VALUES (%s, %s, %s, %s, 0, 0, 0, 0, %s, %s, 0, NOW(), NOW())
      ON CONFLICT (guild_id, url_key) DO UPDATE SET like_count = lockhart_track_intelligence.like_count + %s,
        dislike_count = lockhart_track_intelligence.dislike_count + %s, updated_at = NOW()]],
    guild_id, uk, url, title, delta_like, delta_dislike, delta_like, delta_dislike)
  local score_delta = liked and 4.0 or -4.0
  q([[INSERT INTO lockhart_user_track_affinity (guild_id, user_id, url_key, video_url, title, queued_count, play_count, finish_count, skip_count, like_count, dislike_count, score, last_requested, updated_at)
      VALUES (%s, %s, %s, %s, %s, 0, 0, 0, 0, %s, %s, %s, NOW(), NOW())
      ON CONFLICT (guild_id, user_id, url_key) DO UPDATE SET
        like_count = lockhart_user_track_affinity.like_count + %s, dislike_count = lockhart_user_track_affinity.dislike_count + %s,
        score = lockhart_user_track_affinity.score + %s, updated_at = NOW()]],
...
local function pick_smart_recommendation(guild_id, listener_id)
  if listener_id then
    local row = q1([[SELECT video_url, title FROM lockhart_user_track_affinity
  ...
  local row = q1([[SELECT video_url, title FROM lockhart_track_intelligence
...
local function build_user_taste_summary(guild_id, user_id)
  local rows = q([[SELECT title, score FROM lockhart_user_track_affinity WHERE guild_id = %s AND user_id = %s ORDER BY score DESC LIMIT 8]], guild_id, user_id)
  local agg = q1([[SELECT COALESCE(SUM(play_count),0)::int AS played, COALESCE(SUM(finish_count),0)::int AS finished,
      ...
      COALESCE(SUM(skip_count),0)::int AS skipped FROM lockhart_user_track_affinity WHERE guild_id = %s AND user_id = %s]], guild_id, user_id)
  return rows, agg
end
...
bot.lavalink:on("event", function(msg)
  ...
  elseif msg.type == "TrackEndEvent" then
    ...
    if msg.reason == "finished" then
      ...
      if url then
        local uk = track_key(url, title)
        q("UPDATE lockhart_track_intelligence SET finish_count = finish_count + 1, total_listen_seconds = total_listen_seconds + %s, updated_at = NOW() WHERE guild_id = %s AND url_key = %s",
    ...
    elseif msg.reason == "loadFailed" or msg.reason == "cleanup" then
      ...
      if url then
        local uk = track_key(url, title)
        q("UPDATE lockhart_track_intelligence SET skip_count = skip_count + 1, updated_at = NOW() WHERE guild_id = %s AND url_key = %s", guild_id, uk)
      end
    end
...
local function cmdname(n) return "lockhart_main_" .. n end
...
bot:command(cmdname("sethome"), {
  ...
  q([[INSERT INTO lockhart_bot_home_channels (guild_id, bot_name, home_vc_id) VALUES (%s, 'lockhart', %s)
...
bot:command(cmdname("setfeedback"), {
  ...
  q("UPDATE lockhart_guild_settings SET feedback_channel_id = %s WHERE guild_id = %s", channel_id, guild_id_of(interaction))
...
bot:command(cmdname("djrole"), {
  ...
  ensure_guild_settings(guild_id_of(interaction))
  q("UPDATE lockhart_guild_settings SET dj_role_id = %s WHERE guild_id = %s", role_id, guild_id_of(interaction))
...
bot:command(cmdname("removedj"), {
  ...
  ensure_guild_settings(guild_id_of(interaction))
  q("UPDATE lockhart_guild_settings SET dj_role_id = NULL WHERE guild_id = %s", guild_id_of(interaction))
...
bot:command(cmdname("djmode"), {
  ...
  local row = q1("SELECT dj_only_mode FROM lockhart_guild_settings WHERE guild_id = %s", gid)
  local new_val = not (row and row.dj_only_mode)
  q("UPDATE lockhart_guild_settings SET dj_only_mode = %s WHERE guild_id = %s", new_val, gid)
...
bot:command(cmdname("247"), {
  ...
  local gid = guild_id_of(interaction)
  ensure_guild_settings(gid)
  local row = q1("SELECT stay_in_vc FROM lockhart_guild_settings WHERE guild_id = %s", gid)
  local new_val = not (row and row.stay_in_vc)
  q("UPDATE lockhart_guild_settings SET stay_in_vc = %s WHERE guild_id = %s", new_val, gid)
...
local function queue_index_choices(guild_id, needle)
  local rows = q("SELECT title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT 200", guild_id)
...
bot:command(cmdname("play"), {
  ...
  if not channel_id then
    ...
    local row = q1("SELECT connected_channel_id FROM lockhart_voice_state WHERE guild_id = %s AND bot_name = 'lockhart'", gid)
    channel_id = row and row.connected_channel_id
  end
  if not channel_id then
    followup_embed(interaction, embed("Join a channel first or set a home channel with /lockhart_main_sethome.", { title = "\xE2\x9D\x8C Error", color = COLOR.red }))
    return
  end
...
-- reconnect_attempts column (lockhart_voice_state) and the playback_state
-- reset columns directly, without needing a live Discord interaction.
local function clear_playback_state(guild_id)
  q([[UPDATE lockhart_playback_state SET channel_id = NULL, video_url = NULL, title = NULL, position_seconds = 0,
      is_playing = false, is_paused = false, play_session_key = NULL, track_uid = NULL WHERE guild_id = %s AND bot_name = 'lockhart']], guild_id)
end

local function mark_voice_connected(guild_id, channel_id)
  q([[INSERT INTO lockhart_voice_state (guild_id, bot_name, connected_channel_id, desired_connected, last_seen_at, reconnect_attempts)
      VALUES (%s, 'lockhart', %s, true, NOW(), 0)
...
local function mark_voice_disconnected(guild_id)
  q([[UPDATE lockhart_voice_state SET desired_connected = false, connected_channel_id = NULL, disconnected_at = NOW() WHERE guild_id = %s AND bot_name = 'lockhart']], guild_id)
end

-- Runs periodically, not just at boot, so a guild whose queue-drain stalled
-- out (e.g. every remaining resolve attempt failed during a Lavalink/
-- network outage, or a Lavalink restart dropped the player silently) gets
-- resumed automatically instead of staying dead until someone notices and
-- manually reissues a command.
local function recovery_watchdog()
  local stalled = q("SELECT DISTINCT guild_id FROM lockhart_queue WHERE bot_name = 'lockhart'") or {}
  for _, row in ipairs(stalled) do
    local guild_id = tostring(row.guild_id)
    if not playback[guild_id] then
      local vrow = q1("SELECT connected_channel_id FROM lockhart_voice_state WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
...
function(_, interaction)
  ...
  stop_playback(gid)
  clear_active_playlist(gid)
  q("DELETE FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart'", gid)
  q("DELETE FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart'", gid)
...
bot:command(cmdname("queue"), {
  ...
  local rows = q("SELECT title, requester_id FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT %s OFFSET %s", gid, per_page, offset)
  if #rows == 0 then
...
index_command("remove", "Remove a queued track by its queue number so it will not play later.", true, function(interaction, index)
  ...
  local row = q1("SELECT id, video_url, title, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT 1 OFFSET %s", gid, index - 1)
  if not row then ack(interaction, "Invalid index.", true); return end
  q("DELETE FROM lockhart_queue WHERE id = %s AND guild_id = %s AND bot_name = 'lockhart'", row.id, gid)
...
index_command("skipto", "Drop everything before a chosen queue position and jump playback forward to that track.", true, function(interaction, index)
  ...
  local rows = q("SELECT video_url, title, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT %s", gid, skip_n)
  if #rows > 0 then
    q("DELETE FROM lockhart_queue WHERE id IN (SELECT id FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT %s)", gid, skip_n)
...
bot:command(cmdname("move"), {
  ...
  local rows = q("SELECT id, video_url, title, requester_id, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", gid)
...
index_command("bump", "Move a queued track to the front so it plays next", true, function(interaction, index)
  local gid = guild_id_of(interaction)
  if index < 1 then ack(interaction, "Invalid index.", true); return end
  local row = q1("SELECT id, video_url, title, requester_id, track_uid FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT 1 OFFSET %s", gid, index - 1)
  if not row then ack(interaction, "Invalid index.", true); return end
  q("DELETE FROM lockhart_queue WHERE id = %s AND guild_id = %s AND bot_name = 'lockhart'", row.id, gid)
...
function(_, interaction)
  ...
  local row = q1("SELECT COUNT(*)::int AS c FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' AND requester_id = %s", gid, uid)
  local removed = row and row.c or 0
  if removed > 0 then
    q("DELETE FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' AND requester_id = %s", gid, uid)
    q("DELETE FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart' AND requester_id = %s", gid, uid)
  end
...
function(_, interaction)
  local uid = interaction.member and interaction.member.user and interaction.member.user.id
  local rows = q("SELECT playlist_name, COUNT(*)::int AS c FROM lockhart_user_playlists WHERE user_id = %s GROUP BY playlist_name ORDER BY playlist_name ASC", uid)
...
bot:command(cmdname("deleteplaylist"), {
  ...
  local row = q1("SELECT COUNT(*)::int AS c FROM lockhart_user_playlists WHERE user_id = %s AND playlist_name = %s", uid, name)
  local count = row and row.c or 0
  if count == 0 then ack(interaction, "That playlist was not found.", true); return end
  q("DELETE FROM lockhart_user_playlists WHERE user_id = %s AND playlist_name = %s", uid, name)
...
bot:command(cmdname("savequeue"), {
  ...
  local name = get_opt(interaction, "name")
  local rows = q("SELECT video_url, title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", gid)
  if #rows == 0 then followup_content(interaction, "Queue is empty!", true); return end
  for _, r in ipairs(rows) do
    q("INSERT INTO lockhart_user_playlists (user_id, playlist_name, video_url, title) VALUES (%s, %s, %s, %s)", uid, name, r.video_url, r.title)
  end
...
bot:command(cmdname("loadqueue"), {
  ...
  local gid, uid = guild_id_of(interaction), interaction.member and interaction.member.user and interaction.member.user.id
  local name = get_opt(interaction, "name")
  local rows = q("SELECT video_url, title FROM lockhart_user_playlists WHERE user_id = %s AND playlist_name = %s", uid, name)
...
function(_, interaction)
  defer(interaction, true)
  local gid = guild_id_of(interaction)
  local rows = q("SELECT title, COUNT(*)::int AS plays FROM lockhart_history WHERE guild_id = %s GROUP BY title ORDER BY plays DESC LIMIT 10", gid)
...
function(_, interaction)
  defer(interaction, true)
  local gid = guild_id_of(interaction)
  local rows = q("SELECT title FROM lockhart_history WHERE guild_id = %s ORDER BY played_at DESC LIMIT 5", gid)
...
bot:command(cmdname("userhistory"), {
  ...
  local rows = q("SELECT id, title, video_url FROM lockhart_history WHERE guild_id = %s AND requester_id = %s ORDER BY played_at DESC LIMIT 10", gid, member_id)
  ...
  ack_embed(interaction, embed(table.concat(lines, "\n"), { title = "\xF0\x9F\x8E\xA7 " .. name .. "'s Play History", color = COLOR.blue, footer = "Use /lockhart_main_steal <user> <number> to add one to the queue!" }), true)
end)
...
bot:command(cmdname("steal"), {
  ...
  local row = q1("SELECT video_url, title FROM lockhart_history WHERE guild_id = %s AND requester_id = %s ORDER BY played_at DESC LIMIT 1 OFFSET %s", gid, member_id, track_number - 1)
  if not row then
...
bot:command(cmdname("recommend"), {
  ...
  q([[INSERT INTO lockhart_smart_recommendations (guild_id, requester_id, seed_title, seed_url, query_text, chosen_url, chosen_title, reason, accepted, created_at)
...
bot:command(cmdname("taste"), {
  ...
  local e = embed(table.concat(lines, "\n"), { title = "LOCKHART Taste Profile: " .. name, color = COLOR.blurple })
  agg = agg or {}
...
bot:command(cmdname("volume"), {
  ...
  ensure_guild_settings(gid)
  q("UPDATE lockhart_guild_settings SET volume = %s WHERE guild_id = %s", vol, gid)
...
bot:command(cmdname("loop"), {
  ...
  ensure_guild_settings(gid)
  q("UPDATE lockhart_guild_settings SET loop_mode = %s WHERE guild_id = %s", mode, gid)
...
bot:command(cmdname("filter"), {
  ...
  if stack == nil then
    q("UPDATE lockhart_guild_settings SET filter_mode = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", mode, gid)
  else
    q("UPDATE lockhart_guild_settings SET filter_mode = %s, filter_stack = %s, custom_speed = 1.0, custom_pitch = 1.0, custom_modifiers_left = 0 WHERE guild_id = %s", mode, stack_value, gid)
  end
  if playback[gid] then
    local filters = {}
...
bot:command(cmdname("fade"), {
  ...
  defer(interaction, true)
  ensure_guild_settings(gid)
  q("UPDATE lockhart_guild_settings SET transition_mode = %s, fade_seconds = %s, fade_curve = %s WHERE guild_id = %s", mode, seconds, curve, gid)
  local message, color
  if mode == "smart" then
...
bot:command(cmdname("modify"), {
  ...
  if s.filter_mode ~= "none" then
    followup_embed(interaction, embed("\xE2\x9D\x8C **Conflict:** Disable standard Filters via /lockhart_main_filter none first.", { color = COLOR.red }))
    return
  end
  q("UPDATE lockhart_guild_settings SET custom_speed = %s, custom_pitch = %s, custom_modifiers_left = %s WHERE guild_id = %s", speed, pitch, duration, gid)
  if playback[gid] then
...
local function build_panel_embed(gid)
  local s = get_settings(gid)
  local e = embed(nil, { title = "\xF0\x9F\x8E\x9B\xEF\xB8\x8F LOCKHART Music Control Panel", color = 0x2B2D31 })
  local data = playback[gid]
  e.fields = {}
  if data then
  ...
  e.footer = { text = "LOCKHART Main Music System" }
  return e
end

local PANEL_COMPONENTS = {
  { type = 1, components = {
    { type = 2, style = 1, label = "\xE2\x8F\xAF\xEF\xB8\x8F Play/Pause", custom_id = "lockhart:panel:playpause" },
    { type = 2, style = 4, label = "\xE2\x8F\xB9\xEF\xB8\x8F Stop", custom_id = "lockhart:panel:stop" },
    { type = 2, style = 2, label = "\xE2\x8F\xAD\xEF\xB8\x8F Skip", custom_id = "lockhart:panel:skip" },
  } },
  { type = 1, components = {
    { type = 2, style = 2, label = "\xE2\x8F\xAA -10s", custom_id = "lockhart:panel:rw" },
    { type = 2, style = 2, label = "\xE2\x8F\xA9 +10s", custom_id = "lockhart:panel:fw" },
    { type = 2, style = 2, label = "\xF0\x9F\x94\x89 Vol-", custom_id = "lockhart:panel:voldown" },
    { type = 2, style = 2, label = "\xF0\x9F\x94\x8A Vol+", custom_id = "lockhart:panel:volup" },
  } },
  { type = 1, components = {
    { type = 2, style = 2, label = "\xF0\x9F\x94\x81 Loop", custom_id = "lockhart:panel:looptoggle" },
    { type = 2, style = 3, label = "\xF0\x9F\x94\x80 Shuffle", custom_id = "lockhart:panel:shuffle" },
    { type = 2, style = 2, label = "\xF0\x9F\x93\x9C View Queue", custom_id = "lockhart:panel:queue" },
  } },
}
...
bot:command(cmdname("help"), { description = "Show a categorized help menu for all LOCKHART music commands and utilities" },
function(_, interaction)
...
bot.gateway:on("INTERACTION_CREATE", function(interaction)
  ...
  if interaction.type == 3 then -- MESSAGE_COMPONENT (panel buttons)
    ...
    local action = cid:match("^lockhart:panel:(.+)$")
    ...
    -- never reach lockhart_error_events / the error webhook.
    ...
    elseif action == "looptoggle" then
      ...
      q("UPDATE lockhart_guild_settings SET loop_mode = %s WHERE guild_id = %s", next_mode, gid)
      refresh()
    ...
    elseif action == "queue" then
      local rows = q("SELECT title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC LIMIT 10", gid)
...
-- A much smaller cousin of lockhart.py's dozen tasks.loop()s -- this LuaJIT
...
-- directly and exercise the lockhart_metrics %s-placeholder-count/arg-count
...
local function persist_positions_tick()
  for gid, data in pairs(playback) do
    if not data.paused then
      local ok = pcall(function()
        q([[UPDATE lockhart_playback_state SET position_seconds = %s, last_checkpoint_at = NOW()
            WHERE guild_id = %s AND bot_name = 'lockhart']], current_position(gid), gid)
      end)
...
local function heartbeat_tick()
  q([[INSERT INTO swarm_health (bot_name, last_pulse, status) VALUES ('lockhart', NOW(), 'online')
      ON CONFLICT (bot_name) DO UPDATE SET last_pulse = NOW(), status = 'online']])
  for gid, data in pairs(playback) do
    q([[INSERT INTO lockhart_metrics (guild_id, bot_name, voice_connected, player_connected, player_playing, player_paused,
          queue_count, backup_queue_count, is_playing_db, is_paused_db, position_seconds, duration_seconds, lavalink_ready, updated_at)
        VALUES (%s, 'lockhart', true, true, %s, %s, %s, 0, %s, %s, %s, %s, true, NOW())
...
local function poll_swarm_overrides()
  local rows = q("SELECT guild_id, command FROM lockhart_swarm_overrides WHERE bot_name = 'lockhart'") or {{}}
  for _, row in ipairs(rows) do
    local guild_id = tostring(row.guild_id)
    local cmd_name = (row.command or ""):upper()
    local executed = false
    if cmd_name == "RESTART" then
      q("DELETE FROM lockhart_swarm_overrides WHERE guild_id = %s AND bot_name = 'lockhart'", guild_id)
    ...
    if executed then
      q("DELETE FROM lockhart_swarm_overrides WHERE guild_id = %s AND bot_name = 'lockhart' AND command = %s", guild_id, row.command)
    end
  end
end
...
-- seek-to-position) land in lockhart_swarm_direct_orders; nothing consumed
...
local function handle_direct_order(order)
  ...
  local ok, err = pcall(function()
    if cmd == "PLAY" and order.data and order.vc_id and order.vc_id ~= "0" then
      local tracks, playlist_name, terr = search_playables(order.data)
      if not tracks or #tracks == 0 then error("could not resolve source: " .. tostring(terr), 0) end
      for _, t in ipairs(tracks) do
        enqueue_track(guild_id, t.uri, t.title, nil)
      end
      if #tracks > 1 then shuffle_queue(guild_id, true) end
      ...
      if playlist_name and is_playlist_source(order.data) then
        set_active_playlist(guild_id, order.data, tracks, nil, order.vc_id)
      end
      snapshot_backup(guild_id)
  ...
  if not ok then
    q("UPDATE lockhart_swarm_direct_orders SET attempts = attempts + 1, last_error = %s WHERE id = %s", tostring(err):sub(1, 500), order.id)
  else
    q("DELETE FROM lockhart_swarm_direct_orders WHERE id = %s", order.id)
...
local function poll_direct_orders()
  local rows = q("SELECT id, guild_id, vc_id, text_channel_id, command, data FROM lockhart_swarm_direct_orders WHERE bot_name = 'lockhart' AND claimed_at IS NULL ORDER BY id ASC LIMIT 5") or {}
  for _, order in ipairs(rows) do
    q("UPDATE lockhart_swarm_direct_orders SET claimed_at = NOW() WHERE id = %s", order.id)
...
local function start_background_loops()
  ...
  -- Mirrors lockhart.py's "Database Janitor" retention pass for error_events
  ...
  copas.addthread(function()
    while true do
      copas.sleep(6 * 3600)
      local ok, err = pcall(q, "DELETE FROM lockhart_error_events WHERE created_at < NOW() - INTERVAL '30 days'")
...
bot.gateway:on("READY", function()
  ...
  send_webhook_log("\xF0\x9F\x9F\xA2 Node Online", "LOCKHART is online and syncing with the swarm.", COLOR.green)
  ...
  copas.addthread(function()
    ...
    local rows = q("SELECT DISTINCT guild_id FROM lockhart_bot_home_channels WHERE bot_name = 'lockhart'") or {}
    for _, row in ipairs(rows) do
      local gid = row.guild_id
      local channel_id = get_home_channel_id(gid)
      if channel_id then
        local has_queue = queue_count(gid) > 0
        local was_playing = q1("SELECT is_playing FROM lockhart_playback_state WHERE guild_id = %s AND bot_name = 'lockhart'", gid)
        if has_queue or (was_playing and was_playing.is_playing) then
          log_info("[%s] resuming on boot (queue=%s, was_playing=%s)", tostring(gid), tostring(has_queue), tostring(was_playing and was_playing.is_playing))
          ensure_voice_connection(gid, channel_id)
...
-- nexus.lua/sapphire.lua/symphony.lua's LOCKHART_DRY_RUN=test export pattern --
...
if env("LOCKHART_DRY_RUN") == "test" then
  return {
    q = q, q1 = q1, col = col,
...
elseif env("LOCKHART_DRY_RUN") then
  log_info("dry run OK: %d commands registered, db/lavalink/gateway objects constructed.", command_count())
  os.exit(0)
end

start_background_loops()
log_info("LOCKHART booting -- %d commands registered, Lavalink at %s:%d", command_count(), LL_HOST, LL_PORT)
