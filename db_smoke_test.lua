-- Live-DB smoke test for lockhart.lua's Postgres write paths (mirrors
-- tunestream/nexus/sapphire/symphony's db_smoke_test.lua pattern). Unlike
-- those bots, discord_music_lockhart started completely empty -- no prior
-- MariaDB migration, no pre-existing tables -- so this run also exercises
-- init_db()'s bootstrap_schema() (CREATE TABLE IF NOT EXISTS for every
-- lockhart_* table + swarm_health) as its first order of business, via
-- lockhart.lua's own dofile() at require time below. Every test below uses a
-- distinct, clearly-fake TEST_GUILD / TEST_USER* id and cleans up its own
-- rows at the end. There is no real pre-existing guild data in this database
-- to guard against disturbing (confirmed empty via `\dt` before this port),
-- so -- unlike the sibling bots' smoke tests -- there is no "pre-existing
-- real guild row untouched" guard-rail check at the end.
--
-- Run with: LOCKHART_DRY_RUN=test LOCKHART_DB_PORT=5432 LOCKHART_DB_NAME=discord_music_lockhart luajit db_smoke_test.lua

package.path = "./lib/?.lua;./lib/?/init.lua;" .. package.path

-- Expects LOCKHART_DRY_RUN=test in the environment already (set by the caller,
-- e.g. `LOCKHART_DRY_RUN=test LOCKHART_DB_PORT=5432 luajit db_smoke_test.lua`).
local A = dofile("lockhart.lua")

local TEST_GUILD = "999900021"
local TEST_USER = "999900022"
local TEST_USER2 = "999900023"
...
A.q("UPDATE lockhart_guild_settings SET volume = %s, loop_mode = %s WHERE guild_id = %s", 77, "song", TEST_GUILD)
...
local rows = A.q("SELECT video_url, title FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart' ORDER BY id ASC", TEST_GUILD)
...
local backup_count = A.q1("SELECT COUNT(*)::int AS n FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
A.q("DELETE FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
local backup_after = A.q1("SELECT COUNT(*)::int AS n FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
-- counter column declared NOT NULL DEFAULT 0, but Postgres never auto-fills
-- a value for a column an INSERT's column-list omits entirely -- it only
-- applies the DEFAULT when the column is *named* with no value, so every
-- INSERT below still must name and zero-fill each counter explicitly).
...
local ti = A.q1("SELECT queued_count, play_count, finish_count, like_count, dislike_count, skip_count, total_listen_seconds FROM lockhart_track_intelligence WHERE guild_id = %s AND url_key = %s", TEST_GUILD, key_a)
...
local aff = A.q1("SELECT queued_count, play_count, finish_count, skip_count, like_count, dislike_count, score FROM lockhart_user_track_affinity WHERE guild_id = %s AND user_id = %s AND url_key = %s", TEST_GUILD, TEST_USER, key_a)
...
local ti2 = A.q1("SELECT queued_count FROM lockhart_track_intelligence WHERE guild_id = %s AND url_key = %s", TEST_GUILD, A.track_key("https://example.com/a", "Track A"))
...
A.q([[INSERT INTO lockhart_playback_state (guild_id, bot_name, channel_id, video_url, position_seconds, is_playing, is_paused, title, play_session_key, track_uid)
      VALUES (%s, 'lockhart', %s, %s, 42, true, false, %s, %s, %s)
...
local ps = A.q1("SELECT position_seconds, is_playing FROM lockhart_playback_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
ps = A.q1("SELECT video_url, is_playing, position_seconds FROM lockhart_playback_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
-- confirming mark_voice_connected's explicit zero-fill still matters even
-- though the column has a DEFAULT, since the INSERT below is the one that
...
local vs = A.q1("SELECT connected_channel_id::text AS connected_channel_id, desired_connected, reconnect_attempts FROM lockhart_voice_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
vs = A.q1("SELECT connected_channel_id, desired_connected FROM lockhart_voice_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
check("voice_state disconnected row correct", vs and vs.connected_channel_id == nil and vs.desired_connected == false, tostring(vs))

-- heartbeat_tick / lockhart_metrics (the %s-placeholder-count vs args-supplied
...
-- hitting current_position's duration clamp (200s).
...
local metrics = A.q1("SELECT queue_count, is_playing_db, is_paused_db, position_seconds, duration_seconds FROM lockhart_metrics WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
check("lockhart_metrics row written", metrics ~= nil, tostring(metrics))
check("lockhart_metrics is_paused_db is boolean false, not a position/duration value", metrics and metrics.is_paused_db == false, metrics and tostring(metrics.is_paused_db))
check("lockhart_metrics position_seconds is a plausible position (current_position, not duration/title)", metrics and tonumber(metrics.position_seconds) ~= nil and tonumber(metrics.position_seconds) < 300, metrics and tostring(metrics.position_seconds))
check("lockhart_metrics duration_seconds == 200 (not swapped with position)", metrics and tonumber(metrics.duration_seconds) == 200, metrics and tostring(metrics.duration_seconds))
...
local pst = A.q1("SELECT position_seconds FROM lockhart_playback_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
...
local ee = A.q1("SELECT title, error_type FROM lockhart_error_events WHERE guild_id = %s AND error_type = 'smoke_test' ORDER BY created_at DESC LIMIT 1", TEST_GUILD)
...
local sh = A.q1("SELECT status FROM swarm_health WHERE bot_name = 'lockhart'")
check("swarm_health row written by heartbeat_tick", sh and sh.status == "online", tostring(sh))

-- search_cache table exists and is reachable (lockhart_search_cache -- the
-- one table in this schema not carried over from strife.lua's bootstrap,
-- added fresh here since tunestream.lua's search_playables() caches resolved
-- queries there).
local ok17, err17 = pcall(function()
  A.q([[INSERT INTO lockhart_search_cache (cache_key, resolved_uri, title, cached_at, expires_at)
      VALUES (%s, %s, %s, NOW(), NOW() + INTERVAL '15 minutes')
      ON CONFLICT (cache_key) DO UPDATE SET resolved_uri = EXCLUDED.resolved_uri, title = EXCLUDED.title]],
    "smoketest:cache:key", "https://example.com/cached", "Cached Track")
end)
check("lockhart_search_cache insert runs", ok17, err17)
local sc = A.q1("SELECT resolved_uri, title FROM lockhart_search_cache WHERE cache_key = %s", "smoketest:cache:key")
check("lockhart_search_cache row persisted", sc and sc.resolved_uri == "https://example.com/cached", tostring(sc))
A.q("DELETE FROM lockhart_search_cache WHERE cache_key = %s", "smoketest:cache:key")

-- cleanup: remove every row this test wrote, and ONLY rows scoped to
-- TEST_GUILD/TEST_USER*.
A.q("DELETE FROM lockhart_queue WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
A.q("DELETE FROM lockhart_queue_backup WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
A.q("DELETE FROM lockhart_playback_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
A.q("DELETE FROM lockhart_guild_settings WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM lockhart_swarm_toggles WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM lockhart_track_intelligence WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM lockhart_user_track_affinity WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM lockhart_voice_state WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)
A.q("DELETE FROM lockhart_error_events WHERE guild_id = %s", TEST_GUILD)
A.q("DELETE FROM lockhart_metrics WHERE guild_id = %s AND bot_name = 'lockhart'", TEST_GUILD)

local leftover = A.q1([[
  SELECT
    (SELECT COUNT(*) FROM lockhart_queue WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_queue_backup WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_playback_state WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_guild_settings WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_track_intelligence WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_user_track_affinity WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_voice_state WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_error_events WHERE guild_id = %s) +
    (SELECT COUNT(*) FROM lockhart_metrics WHERE guild_id = %s) AS n
